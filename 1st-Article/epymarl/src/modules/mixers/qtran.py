...
class QTranBase(nn.Module):
    ...
    def forward(self, batch, hidden_states, actions=None):
        ...
        # Normalize state to [0,1] range — raw values (positions 0-100, energy 0-100)
        # cause network outputs to overflow on MPS
        states = states / 100.0
        ...
        states_v = batch["state"].reshape(bs * ts, self.state_dim) / 100.0
        v_outputs = self.V(states_v)
