"""Test: run QTRAN without np.random.seed to see if PDR improves."""
...
# DO NOT seed numpy globally
torch.manual_seed(42)

env = env_REGISTRY['gymma'](key='gym_examples:WSNRouting-v0', time_limit=30, pretrained_wrapper=None, seed=42)
info = env.get_env_info()

scheme = {
    'state': {'vshape': info['state_shape']}, 'obs': {'vshape': info['obs_shape'], 'group': 'agents'},
...
print('=== No np.random.seed, 50 episodes ===')
pdrs = []
for ep in range(50):
    env.reset()
...
print(f'Mean PDR: {np.mean(pdrs)*100:.1f}%')
print(f'>50%: {sum(1 for p in pdrs if p>0.5)}, >20%: {sum(1 for p in pdrs if p>0.2)}, ==0: {sum(1 for p in pdrs if p==0)}')

# Now test WITH np.random.seed(42)
print('\n=== WITH np.random.seed(42), 50 episodes ===')
np.random.seed(42)
env2 = env_REGISTRY['gymma'](key='gym_examples:WSNRouting-v0', time_limit=30, pretrained_wrapper=None, seed=42)
pdrs2 = []
for ep in range(50):
    env2.reset()
    mac.init_hidden(batch_size=1)
    ...
    for t in range(30):
        if done:
            break
        batch.update({'state': [env2.get_state()], 'avail_actions': [env2.get_avail_actions()], 'obs': [env2.get_obs()]}, ts=t)
        actions = mac.select_actions(batch, t_ep=t, t_env=t, test_mode=True)
        reward, done, info2 = env2.step(actions[0])
        ret += reward
        terminated_flag = done and not info2.get('episode_limit', False)
        batch.update({'actions': actions.unsqueeze(-1), 'reward': [(reward,)], 'terminated': [(terminated_flag,)]}, ts=t)
    pdr = env2.original_env.__dict__['env'].__dict__['packet_delivery_ratio']
    pdrs2.append(pdr)

print(f'Mean PDR: {np.mean(pdrs2)*100:.1f}%')
print(f'>50%: {sum(1 for p in pdrs2 if p>0.5)}, >20%: {sum(1 for p in pdrs2 if p>0.2)}, ==0: {sum(1 for p in pdrs2 if p==0)}')
