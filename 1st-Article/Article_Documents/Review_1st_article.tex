...
\FOR{$k = 1, \dots, K$}
    \STATE Reset $\mathcal{M}$; build $\hat{\mathbf{A}}_k$ (Eq.~\ref{eq:norm_adj}), $\mathbf{X}_k$ (Eq.~\ref{eq:node_features}); compute $\mathbf{z}_k$ via GCN (Eq.~\ref{eq:gcn})
...
He received a bachelor’s degree in software engineering from Polytechnique Montréal in $2021$.
\end{IEEEbiography}
