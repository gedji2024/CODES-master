...
% [RÉVISÉ — C1a] Macro bstctlcite supprimée : elle injectait deux entrées
% bibliographiques fantômes décalant toutes les références de 2 positions.
% Cette macro n'est utile que pour la soumission aux revues IEEE, pas pour la thèse.
...
\graphicspath{{./images/}{./dia/}{./gnuplot/}{./figures/}}
%%
...
\PassOptionsToPackage{hyphens}{url}  % Coupures dans URLs longues
...
\usepackage{microtype}  % Justification améliorée, réduit les débordements
\usepackage{hyperref}
...
\usepackage{listings} % Pour les lignes de code / For code listings
\usepackage{booktabs} % Pour \toprule, \midrule, \bottomrule
\usepackage{multirow} % Pour les cellules multi-lignes dans les tableaux
\usepackage{enumitem} % Pour personnaliser les listes
\usepackage{pifont} % Pour \ding{51} (checkmark)
\newcommand{\cmark}{\ding{51}}
\newcommand{\xmark}{\ding{55}}
\newcommand{\pmark}{{\footnotesize$\circ$}}
\usepackage{tikz} % Pour les diagrammes
\usetikzlibrary{positioning, arrows.meta, decorations.pathreplacing, calc, shapes.geometric, fit}
\usepackage{pgfplots} % Pour les graphiques
\pgfplotsset{compat=1.18}
\usepackage{algorithm} % Pour les algorithmes (float)
\usepackage{algpseudocode} % Pour les pseudocodes (algorithmic)
% Aliases pour compatibilité avec le style algorithmic classique (Article 1)
\providecommand{\REQUIRE}{\Require}
\providecommand{\ENSURE}{\Ensure}
\providecommand{\STATE}{\State}
\providecommand{\COMMENT}[1]{\hfill\(\triangleright\) \textit{#1}}
...
\usepackage{amsthm} % Pour les environnements théorème
...
\begin{document}
% [RÉVISÉ — C1a] \bstctlcite supprimé — voir commentaire ci-dessus.
...
\include{AvantPropos}         % Avant-propos : statut de publication des articles + note revue IEEE TGCN
...
\include{4-Sigles_Abrev_REVISED}  % [RÉVISÉ — C2a] Liste des symboles mathématiques ajoutée.
\ifthenelse{\equal{\AnnexesPresentes}{O}}{\listofappendices}{}
\mainmatter
\include{5-Introduction_new}       % Introduction au sujet de recherche.
\include{6-Revue_litterature_new}  % [RÉVISÉ — C2b] Section métaheuristiques enrichie, formule Heinzelman en display.
\include{6b-Demarche_new}          % Démarche de l'ensemble du travail de recherche (chapitre de transition)
...
\include{7b-Discussion_new}        % Discussion Générale (chapitre de discussion transversale)
\include{8-Conclusion_new}  % [RÉVISÉ — C6/C8] Titres de sections révisés, recommandations renforcées.
%\backmatter
\ifthenelse{\equal{\Langue}{english}}{
	...
	\bibliography{Document_new_corrected}
