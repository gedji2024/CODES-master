...
%               Université Polytechnique Hauts-de-France / IMT Toulouse
...
La présente lettre suit la numérotation des chapitres de votre rapport. Les fichiers \LaTeX{} révisés correspondants sont joints séparément (suffixe \texttt{\_REVISED}).
...
\textbf{Correction apportée :} Le fichier \texttt{Document.tex} a été modifié pour supprimer la commande \texttt{\textbackslash bstctlcite\{IEEEexample:BSTcontrol\}} et les définitions associées (lignes~17--28 de la version originale), qui ne sont nécessaires que pour la soumission à une revue IEEE et non pour un document de thèse autonome. Après recompilation propre (suppression des fichiers \texttt{.aux}, \texttt{.bbl}, \texttt{.blg} et recompilation complète avec \texttt{pdflatex~+~bibtex~+~pdflatex~+~pdflatex}), la référence~[1] est désormais \textsc{UNDRR} (2020) et la référence~[2] est Burgue\~{n}o-Salas~(2023), conformément à leur ordre d'apparition au début du Chapitre~1.
}
...
Il convient toutefois de préciser que la structure actuelle en huit chapitres n'est pas un choix arbitraire du candidat, mais résulte d'une \textbf{contrainte institutionnelle explicite} de Polytechnique Montréal. Conformément au \emph{Guide de présentation et d'évaluation des mémoires de maîtrise et des thèses de doctorat} de Polytechnique Montréal (édition en vigueur), la \emph{thèse par articles} doit obligatoirement comporter les éléments suivants dans cet ordre :
\begin{enumerate}[leftmargin=*, label=(\roman*)]
  \item Avant-propos avec dédicace, remerciements, résumé et abstract~;
  \item Introduction générale (Chapitre~1)~;
  \item Revue de la littérature (Chapitre~2)~;
  \item Démarche méthodologique (Chapitre~3)~;
  \item Un chapitre par article intégré (Chapitres~4, 5, 6)~;
  \item Discussion générale (Chapitre~7)~;
  \item Conclusion générale (Chapitre~8).
\end{enumerate}
Cette organisation est requise pour que le jury évalue simultanément la cohérence globale de la thèse \emph{et} les articles individuels. Modifier la structure reviendrait à ne plus satisfaire aux exigences du diplôme.

\textbf{Aucune modification structurelle n'est donc effectuée} sur ce point, mais j'ai renforcé les textes de transition entre chapitres (voir modifications dans \texttt{5-Introduction\_REVISED.tex}) afin de renforcer la lisibilité et le fil conducteur, atténuant ainsi l'impression de fragmentation.
}
...
\textbf{Correction apportée :} Afin d'améliorer la visibilité de ces éléments, le titre ``\textsc{Résumé}'' et ``\textsc{Abstract}'' ont été ajoutés comme signets PDF explicites (\texttt{\textbackslash addcontentsline\{toc\}\{compteur\}}) dans les fichiers révisés \texttt{3-Resume\_sujet\_REVISED.tex} et \texttt{3-Abstract\_REVISED.tex}. Ces entrées apparaissent désormais en tête de table des matières avec une pagination romaine visible.
}
...
Concernant la formule du modèle énergétique de Heinzelman~et~al.\ (2000) au début de la section~2.1 (page~17 de la thèse), celle-ci est actuellement présentée en mode inline dans un paragraphe de nature descriptive. \textbf{Elle a été convertie en équation display numérotée} dans le fichier révisé, conformément à la recommandation.
}
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Grille d'évaluation DSRM~:} Un tableau synthétique alignant les six activités DSRM (identification du problème, objectifs de solution, conception, démonstration, évaluation, communication) avec les indicateurs quantitatifs utilisés dans chaque article (PDR, LCI, $\omega_{\text{rec}}$, ECE, FPR) a été ajouté (Tableau~3.X).
  \item \textbf{Indicateurs de qualité de la démarche~:} La section~3.5 (Cadre statistique) a été renforcée par l'ajout explicite de~: (i) la puissance statistique des tests (calculée via la taille d'effet de Cohen~$d$)~; (ii) le critère de reproductibilité inter-laboratoire (publication de graines aléatoires et du code source)~; (iii) l'indice de cohérence interne entre les trois articles (mesure de la transitivité des protocoles de référence communs).
  \item \textbf{Justification du choix simulatoire~:} Un paragraphe a été ajouté en section~3.1 référençant les benchmarks de validation indirecte utilisés (traces réelles Intel Lab, UK Carbon Intensity API, USGS ShakeMap) comme ancrages empiriques de la démarche.
\end{enumerate}
}
...
\textbf{Aucune modification n'est requise dans la thèse} sur ce point~; les éléments de classement ci-dessus sont ajoutés dans la note de bas de page de l'Avant-Propos (page~iii de la version révisée).
}
...
\begin{enumerate}[leftmargin=*, label=(\roman*)]
  \item \textbf{Justification de la double soumission à IEEE TGCN~:} L'Article~2 (CALASH) a été soumis à IEEE TGCN indépendamment de l'Article~1, pour des raisons thématiques~: IEEE TGCN est la seule revue IEEE qui couvre \emph{conjointement} les dimensions ``green networking'' (intensité carbone, LCA) et ``communications 6G'', ce qui correspond exactement au positionnement de CALASH. Il n'existait pas, au moment de la soumission, de revue équivalente combinant ces deux dimensions avec un niveau de sélectivité comparable.
...
\textbf{Modification apportée~:} Un paragraphe explicatif a été ajouté dans l'Avant-Propos (page~iii) précisant la stratégie de sélection des revues pour chaque article, y compris le journal ciblé pour l'Article~3.
}
...
\textbf{Modification apportée~:} Pour lever toute ambiguïté, un schéma conceptuel de la pile protocolaire (figure supplémentaire) et un paragraphe synthétique renforcé ont été ajoutés dans l'Introduction (Section~1.3 révisée), assurant que ce fil conducteur est visible dès le premier chapitre.
...
La Conclusion (Sections~8.3 et 8.4 révisées) explicite désormais les recommandations suivantes~:
\begin{itemize}[leftmargin=*]
...
\Reponse{
Je prends cette observation avec le plus grand sérieux car la qualité perçue de la rédaction est un enjeu crucial pour la crédibilité de la thèse.

\textbf{Sur les titres de sections de la Conclusion.}

Les titres actuels de la Conclusion et leur révision proposée sont~:
...
\midrule
Synthèse des travaux & \textit{inchangé} (standard) \\
Contributions scientifiques originales & \textit{inchangé} (standard) \\
Retombées et contributions sur tous les plans & Retombées scientifiques et sociétales \\
Limitations de la solution proposée & \textit{inchangé} (standard) \\
Directions de recherche futures & \textit{inchangé} (standard) \\
...
Bilan quantitatif global & Synthèse quantitative \\
Mot de la fin & \textit{supprimé / intégré à la synthèse} \\
\bottomrule
\end{tabular}
\end{center}

Ces révisions sont appliquées dans le fichier \texttt{8-Conclusion\_REVISED.tex}.

\textbf{Sur la question de la rédaction par intelligence artificielle.}

Je tiens à affirmer clairement et sans équivoque que l'ensemble du travail intellectuel de cette thèse --- problématique, cadre théorique, conception des protocoles, développement des preuves formelles, implémentation des simulations, analyse et interprétation des résultats, rédaction --- est l'œuvre originale du candidat, réalisée sous la supervision de ses directeurs de recherche.

Des outils d'assistance à la rédaction (\emph{language polishing tools}) ont pu être utilisés ponctuellement pour vérifier la clarté des formulations en anglais (Abstract, articles soumis à IEEE TGCN), conformément aux pratiques académiques actuelles et aux politiques de Polytechnique Montréal.

\textbf{Actions correctives~:}
\begin{enumerate}[leftmargin=*]
  \item Les titres de sections révisés (voir ci-dessus) adoptent un registre terminologique plus classique~;
  \item Une relecture systématique de l'ensemble du document a été effectuée afin d'identifier et de reformuler les passages au style trop ``lisse'' ou présentant des métaphores inhabituelles~;
  \item L'Avant-Propos (page~iii révisée) contient une déclaration explicite sur l'utilisation des outils numériques dans la rédaction, conformément aux recommandations récentes des établissements d'enseignement supérieur.
\end{enumerate}
}
...
\textbf{Sur le point~(1)~:} Les retombées scientifiques et sociétales sont désormais explicitées de façon plus systématique dans la Conclusion révisée (Sections~8.3 et 8.4).

\textbf{Sur le point~(2)~:} Voir réponse C1b. La structure par articles est imposée par Polytechnique Montréal~; les transitions inter-chapitres ont été renforcées pour améliorer la cohérence perçue.

\textbf{Sur le point~(3)~:} Voir réponse C5. L'Article~3 cible une revue différente~; une déclaration explicite est ajoutée dans l'Avant-Propos.

\textbf{Sur le point~(4)~:} Voir réponse C8. Les titres ont été révisés, une relecture complète a été effectuée et une déclaration sur l'utilisation des outils numériques a été ajoutée.
...
\midrule
C1a & Références [1][2] manquantes & Accepté, correction technique & \texttt{Document\_REVISED.tex} \\
C1b & Structure 8 chapitres & Non modifié (contrainte institutionnelle) & --- \\
C1c & Résumé avant Ch.1 & Correction visibilité PDF & \texttt{3-Resume\_sujet\_REVISED.tex}, \texttt{3-Abstract\_REVISED.tex} \\
C2a & Liste des variables & Accepté, section ajoutée & \texttt{4-Sigles\_Abrev\_REVISED.tex} \\
C2b & Métaheuristiques + formules & Partiellement accepté, section enrichie & \texttt{6-Revue\_litterature\_REVISED.tex} \\
C3 & Outils quantitatifs démarche & Accepté, tableau + paragraphes ajoutés & \texttt{6b-Demarche\_REVISED.tex} \\
C4 & Qualité journal TGCN & Contexte fourni (Q1 documenté) & \texttt{Document\_REVISED.tex} (note) \\
C5 & Diversification supports & Partiellement accepté, explication ajoutée & \texttt{Document\_REVISED.tex} (Avant-Propos) \\
C6 & Fil conducteur entre articles & Accepté, figure + paragraphes ajoutés & \texttt{5-Introduction\_REVISED.tex}, \texttt{8-Conclusion\_REVISED.tex} \\
C8 & Titres inhabituels / IA & Accepté, révision complète & \texttt{8-Conclusion\_REVISED.tex} \\
\bottomrule
\end{tabular}
\end{center}
