...
\midrule
\multirow{3}{*}{Art.~1 (GAPF)} & N\oe{}ud capteur $i$ & $[RE_i, CE_i, x_i, y_i, N_i]$ $\in \mathbb{R}^{5}$ (énergie résiduelle, énergie consommée, position $x$-$y$, nombre de paquets) & Sélection du prochain saut parmi $\mathcal{N}(v) \cup \{\text{BS}\}$ & $r = \alpha_1 \cdot \text{PDR} + \alpha_2 \cdot (1 - \sigma_E / \bar{E}) + \alpha_3 \cdot \text{alive\_ratio}$ \\
\addlinespace
