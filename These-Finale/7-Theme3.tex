\Chapter{ARTICLE 3~: ZERO-TRUST SHIELDED GRAPH-STRUCTURED DECISION CONTROL FOR SECURE AUTONOMOUS RECOVERY IN AI-NATIVE 6G DISASTER-MONITORING SENSING NETWORKS}\label{sec:Theme3}

\vspace{0.5cm}
\noindent\textbf{Auteurs~:} Georges Parfait Djimefo Kapen, Ranwa Al Mallah et Samuel Pierre.

\noindent\textbf{Revue~:} IEEE Transactions on Mobile Computing.

\noindent\textbf{Statut~:} Soumis.

\vspace{0.3cm}
\noindent\textit{Ce chapitre pr\'esente le troisi\`eme article de cette th\`ese. Il propose CASTER-ZT (Zero-Trust Shielded Decision-Control), un cadre combinant un encodeur par r\'eseau de neurones sur graphe (GNN), une politique de reprise apprise et un \'evaluateur neuronal double hypoth\`ese confiance--risque avec un bouclier d\'eterministe formellement v\'erifi\'e qui impose l'admissibilit\'e born\'ee avant l'ex\'ecution. Dix propri\'et\'es th\'eoriques sont d\'emontr\'ees, incluant une borne de d\'efense en profondeur, une garantie conforme sans distribution et un taux de convergence de calibration. Ce travail constitue la troisi\`eme contribution de la th\`ese en \'etablissant un cadre de s\'ecurit\'e formellement fond\'e pour les d\'ecisions autonomes dans les r\'eseaux de capteurs 6G.}
...
\textbf{RQ4}: Does the AI-native model satisfy 6G near-real-time deployment constraints (Definition~\ref{art3:def:6g_compliance}) at all tested topology scales?
...
Classical (non-deep) anomaly-detection methods---Isolation Forest, One-Class SVM, and DBSCAN---remain the standard unsupervised baselines for network telemetry anomaly detection and IoT intrusion detection~\cite{zahoor2025ifocsvm,haque2023dbscanwsn}; we include an Isolation-Forest-based trust variant (IF-Trust) as an external baseline to explicitly compare neural and classical anomaly-detection approaches within the CASTER-ZT pipeline (Section~\ref{art3:sec:experiments}).
...
(ii)~the ten theoretical propositions establish \emph{bounded and verifiable risk properties} (risk management), including the distribution-free conformal coverage guarantee (Proposition~\ref{art3:prop:conformal}) that regulators can independently verify;
...
Table~\ref{art3:tab:model_positioning} summarizes the positioning of CASTER-ZT against relevant model families.
...
\label{art3:tab:model_positioning}
\resizebox{\textwidth}{!}{%
...
Section~\ref{art3:sec:experiments} validates this positioning empirically: the CPO-Soft, Shield-Binary, and Agentic-Auto baselines confirm the gaps indicated by the \xmark{} and \pmark{} entries in their respective model-family rows.
...
At \textsc{medium} severity, up to $p_{\mathrm{poison}}^{(\mathrm{med})}$ of records in the target zone are poisoned with bias magnitude $b^{(\mathrm{med})}$; at \textsc{high} severity, $p_{\mathrm{poison}}^{(\mathrm{high})}$ with bias $b^{(\mathrm{high})}$ (concrete values in Section~\ref{art3:sec:experiments}).
...
At \textsc{medium} severity, $n_{\mathrm{rogue}}^{(\mathrm{med})}$ rogue actions per tick with unknown identities; at \textsc{high}, $n_{\mathrm{rogue}}^{(\mathrm{high})}$ rogue actions using stolen legitimate identities (concrete values in Section~\ref{art3:sec:experiments}).
...
\begin{itemize}
    ...
    \item \textbf{Integrity (authentication).}  The authorization gate verifies identity legitimacy ($\alpha_t$) for every action proposal before it crosses the admissibility boundary, providing per-action authentication at the actuation layer (validated empirically in Section~\ref{art3:sec:experiments}).
    \item \textbf{Availability.}  The graduated five-outcome decision space (\textsc{allow}, \textsc{scope-reduce}, \textsc{defer}, \textsc{escalate}, \textsc{block}) is explicitly availability-aware: the shield preserves partial service availability via scope reduction rather than indiscriminate hard-blocking.  The bounded price of safety (Proposition~\ref{art3:prop:price_safety}) formally guarantees that availability cost remains bounded (validated empirically in Section~\ref{art3:sec:experiments}).
...
\end{align*}
where $\ell_\theta(a, s_t) := \mathrm{MLP}_\theta([\,h_a \,\|\, \bar{h}_{G_t}\,])$ is a learned scoring function that takes the concatenation of the action embedding $h_a$ and the graph-level readout $\bar{h}_{G_t}$ from the GNN encoder (Section~\ref{art3:sec:architecture}).
The policy is trained end-to-end via supervised imitation on annotated recovery episodes (Section~\ref{art3:sec:training_pipeline}).

\begin{remark}[Learned parameters]
\label{art3:rem:notation}
...
All three parameter sets are trained on annotated recovery episodes before deployment (Section~\ref{art3:sec:training_pipeline}).
The shield algorithm (Algorithm~\ref{art3:alg:casterzt}), calibration procedure (Algorithm~\ref{art3:alg:calibration}), and all ten theoretical properties are defined over the shield's deterministic decision logic and hold identically regardless of how $\theta$, $\phi$, $\omega$ are instantiated---the same shield wraps any learned model.
\end{remark}

\subsubsection{Dual-hypothesis trust--risk evaluation}
\label{art3:sec:dual_hypothesis}
...
The evaluator is implemented as a neural network trained on annotated benign and adversarial episodes (Section~\ref{art3:sec:training_pipeline}).
...
Let $\tilde{x}_t^{(1)} := x_t + \hat{\delta}_t$ denote the \emph{adversarial hypothesis}, where $\hat{\delta}_t$ is a bounded perturbation estimate constructed from the autoencoder reconstruction residual of the trust-assessment network (Section~\ref{art3:sec:trust_assessment}).
\end{definition}
...
Uncertainty: $u_t = U_\omega(s_t, \hat{a}_t) \in [0,1]$, computed via MC-Dropout~\cite{gal2016dropout} over the GNN encoder (Section~\ref{art3:sec:training_pipeline}).
...
\]
where $w_1$ (distrust weight), $w_2$ (risk weight), $w_3$ (uncertainty weight), and $w_4$ (divergence weight) are strictly positive calibration coefficients (Section~\ref{art3:sec:architecture}).
...
All 14 shield parameters ($\gamma_1$--$\gamma_4$, $w_1$--$w_4$, $\kappa_1$, $\kappa_2$, $\tau_0$, $\delta_{\max}$, $\delta_{\max}^{\mathrm{hard}}$, $u_{\max}$) are set by offline calibration (Algorithm~\ref{art3:alg:calibration}).
...
Both constraints are formally validated in the evaluation (Section~\ref{art3:sec:evaluation}, RQ4).
\end{remark}
...
The architecture is shaped by two binding constraints from Definitions~\ref{art3:def:6g_compliance} and~\ref{art3:def:ai_native}.
...
\label{art3:fig:caster_architecture}
\end{figure}


\subsubsection{Main components}
\label{art3:sec:architecture}
...
\begin{enumerate}
    ...
    h_v^{(\ell+1)} = \sigma\!\Big(W^{(\ell)} \!\cdot\! \mathrm{CONCAT}\!\big(h_v^{(\ell)},\; \mathrm{MEAN}(\{h_u^{(\ell)} \!:\! u \!\in\! \mathcal{N}(v)\})\big)\!\Big)
    $}
    \]
    ...
    The policy is trained via supervised imitation on expert-annotated recovery trajectories (Section~\ref{art3:sec:training_pipeline}).
    \item \textbf{Dual-hypothesis trust--risk evaluator} ($Q_\phi$, learned).
    An autoencoder-based anomaly detector for trust scoring and a neural contrastive risk scorer for divergence estimation (Section~\ref{art3:sec:trust_assessment}).
    ...
    The formal admissibility gate (Algorithm~\ref{art3:alg:casterzt}) that receives $(\tau_t, \rho_t, u_t, \Delta_t, \alpha_t)$ and produces a bounded decision $d_t \in \mathcal{D}$.
    The shield uses \emph{no learned parameters}---its 14 scalar parameters ($\gamma_1$--$\gamma_4$, $w_1$--$w_4$, $\kappa_1$, $\kappa_2$, $\tau_0$, $\delta_{\max}$, $\delta_{\max}^{\mathrm{hard}}$, $u_{\max}$) are set by offline calibration (Algorithm~\ref{art3:alg:calibration}).
...
\label{art3:sec:trust_assessment}
...
Algorithm~\ref{art3:alg:calibration} describes the offline calibration procedure for setting shield thresholds.

\begin{algorithm}[t]
\caption{Shield threshold calibration}
\label{art3:alg:calibration}
\begin{algorithmic}[1]
...
Algorithm~\ref{art3:alg:scope_reduce} describes the scope-reduction logic applied when $d_t = \textsc{scope-reduce}$.

\begin{algorithm}[t]
\caption{Scope-reduction enforcement}
\label{art3:alg:scope_reduce}
\begin{algorithmic}[1]
...
\label{art3:tab:complexity_profile}
...
\label{art3:sec:training_pipeline}

This section describes the end-to-end training procedure for the learned components ($\theta$, $\phi$, $\omega$).
The zero-trust shield itself requires no training---its 14 scalar parameters are set by the offline calibration procedure (Algorithm~\ref{art3:alg:calibration}).

\noindent\textbf{Training data generation.}
Training data is generated from 2,000 simulated disaster-recovery episodes (Section~\ref{art3:sec:data_provenance}) spanning all four attack conditions and both severity levels.
...
All architectural hyperparameters are chosen to satisfy the 6G deployment constraints (Definition~\ref{art3:def:6g_compliance}).
...
Dropout ($p = 0.1$) is applied to the GNN layers; during inference, MC-Dropout uses $T = 20$ forward passes with active dropout to estimate epistemic uncertainty $u_t$ (Remark~\ref{art3:rem:notation}).
...
Table~\ref{art3:tab:training_summary} summarizes the learned components and their training.

\begin{table}[t]
\centering
\caption{Training summary for all learned components.}
\label{art3:tab:training_summary}
\begin{tabular}{lccc}
\toprule
...
\noindent
For the 2,420-run evaluation campaign (Section~\ref{art3:sec:experiments}), every learned component executes via \emph{direct forward-pass inference} through the trained PyTorch checkpoints produced by the training pipeline above.
...
Component-level convergence evidence is reported later in the experimental section (Figure~\ref{art3:fig:training_curves}), while this section restricts itself to the training specification needed to reproduce the learned modules.

\noindent\textbf{AI-native coverage and 6G compliance.}
The pipeline architecture specifies five learned neural modules (GNN encoder, policy head, trust autoencoder, risk scorer, contrastive encoder) plus MC-Dropout uncertainty derived from the GNN's dropout masks, yielding AI-native coverage $\eta_{\mathrm{AI}} = 5/6$ (Definition~\ref{art3:def:ai_native}).
The total design has $\approx$20K trainable parameters ($\le 2$\,MB serialized), satisfying the 6G edge-compactness constraint (Definition~\ref{art3:def:6g_compliance}).
After module initialization, the shield thresholds are calibrated on a held-out calibration set (Algorithm~\ref{art3:alg:calibration}), which takes $< 1$ second.
During deployment, inference is a single forward pass through the five modules followed by the deterministic shield decision---designed for per-decision latency of $\approx$3\,ms, within the O-RAN Near-RT RIC 10\,ms control-loop budget (empirically validated in Section~\ref{art3:sec:experiments}).
...
\label{art3:prop:monotone}
Fix $\hat a_t$, $s_t$, and $\alpha_t=1$.
Under Assumption~\ref{art3:asm:standing}, decreasing $\tau_t$ or increasing any of $\rho_t$, $u_t$, $\Delta_t$ cannot move the shield output to a less conservative decision.
\end{proposition}
\begin{proof}
...
\label{art3:prop:authz}
If $\alpha_t = 0$, then $d_t \neq \textsc{allow}$.
\end{proposition}
\begin{proof}
In Algorithm~\ref{art3:alg:casterzt}, the authorization check at line~11 is evaluated before any permissive branch.
...
\label{art3:prop:completeness}
Under Assumption~\ref{art3:asm:standing}, the decision mapping $\Gamma$ is a total function: every valid input $(s_t, \hat{a}_t, \alpha_t, \tau_t, \rho_t, u_t, \Delta_t)$ maps to exactly one $d_t \in \mathcal{D}$.
\end{proposition}
\begin{proof}
Algorithm~\ref{art3:alg:casterzt} is a finite ordered branching structure.
...
\label{art3:prop:complexity}
Under Assumption~\ref{art3:asm:standing}, if the graph encoder uses $L$ message-passing layers with latent dimension $d$, the total per-decision complexity is $\mathcal{O}(L|E_t|d + |\mathcal{A}|k)$.
\end{proposition}
\begin{proof}
...
\begin{remark}
Proposition~\ref{art3:prop:depth} has a direct empirical interpretation.
In the experiments (Section~\ref{art3:sec:experiments}), we observe $\varepsilon_\alpha = 0$ (100\% identity-abuse detection), meaning the authorization gate alone prevents all identity-based unsafe executions.
...
Proposition~\ref{art3:prop:calibration} (Hoeffding) requires i.i.d.\ data and gives an asymptotic concentration bound.
Proposition~\ref{art3:prop:conformal} (conformal) requires only exchangeability and gives an exact finite-sample coverage guarantee.
...
\end{proposition}
\begin{proof}
Algorithm~\ref{art3:alg:calibration} selects $\gamma_1^{(n)}$ as the largest threshold such that the empirical false-allow rate $\hat{\varepsilon}(\gamma_1^{(n)})$ satisfies $\hat{\varepsilon}(\gamma_1^{(n)}) + \sqrt{\ln(1/\delta)/(2n)} \le \varepsilon_{\mathrm{target}}$.
By Hoeffding's inequality (Proposition~\ref{art3:prop:calibration}), the empirical false-allow rate concentrates around the population rate:
\[
...
Proposition~\ref{art3:prop:convergence} provides the convergence guarantee that complements the finite-sample coverage of Proposition~\ref{art3:prop:conformal}.
...
For the inverse sensitivity observed empirically ($L_\varepsilon \approx 2.1$, estimated from the threshold sensitivity sweep in Table~\ref{art3:tab:threshold_sensitivity}), this gives $|\gamma_1^{(1000)} - \gamma_1^\star| \le 0.063$ with 99\% confidence---well within the smooth operating region identified by the threshold sweep.
\end{remark}

\begin{remark}[Union bound for grid-search calibration]
\label{art3:rem:union_bound}
In practice, Algorithm~\ref{art3:alg:calibration} selects the best threshold configuration from a finite grid of $K$ candidate settings.
Applying Proposition~\ref{art3:prop:calibration} with a union bound over $K$ candidates, the simultaneous coverage guarantee becomes
\[
\Pr\!\left[\varepsilon^\star \le \hat{\varepsilon} + \sqrt{\frac{\ln(K/\delta)}{2n}}\right] \ge 1 - \delta.
\]
For the seven-point threshold sweep in Table~\ref{art3:tab:threshold_sensitivity} ($K=7$) with $n=1{,}000$ and $\delta=0.01$, this yields $\varepsilon^\star \le \hat{\varepsilon} + 0.066$---a modest increase of $\sqrt{\ln 7/(2 \cdot 1000)} \approx 0.031$ over the single-threshold bound.
...
\label{art3:sec:evaluation}

\subsection{Evaluation protocol}

\noindent\textbf{Four-layer data design.}
The evaluation uses a four-layer data design to maximize experimental rigor: (1)~synthetic disaster graph simulation generating disrupted sensing/control episodes for both training (Section~\ref{art3:sec:training_pipeline}) and testing, (2)~real-distribution telemetry generation using published 5G measurement distributions---throughput from Narayanan et al.~\cite{narayanan2021first} (LogNormal), latency from Xu et al.~\cite{xu2020understanding} (Gamma), packet-loss rate from 3GPP TR~38.913~\cite{3gpp_tr38913} (Beta), and cell load from Xu et al.~\cite{xu2017bigdata} (Beta)---validated via Kolmogorov--Smirnov goodness-of-fit tests (all $p > 0.68$), (3)~a calibration layer mapping the learned trust and risk signals to bounded shield thresholds via conformal quantile selection (Proposition~\ref{art3:prop:conformal}), and (4)~adversarial injection grounded in attack distributions from the SWaT industrial-control testbed~\cite{goh2017swat}.

\subsubsection{Real-data calibration evidence}
\label{art3:sec:calibration_evidence}

Table~\ref{art3:tab:calibration_evidence} summarizes the specific parameter values extracted from real datasets and used to calibrate the trust-assessment and attack-injection components.
...
\label{art3:tab:calibration_evidence}
\resizebox{\textwidth}{!}{%
\begin{tabular}{p{3.2cm}p{3.2cm}p{2.5cm}p{3.8cm}p{6.5cm}}
\toprule
\textbf{Parameter} & \textbf{Source dataset} & \textbf{Extracted value} & \textbf{Shield parameter} & \textbf{Justification} \\
\midrule
...
\label{art3:sec:real_data_calibration}
...
\label{art3:sec:data_provenance}
...
\begin{enumerate}
    ...
    Nine simulation parameters (Table~\ref{art3:tab:calibration_evidence}) are calibrated against values extracted from four published real-world datasets:
    ...
    These values are directly traceable to the source datasets via the percentiles and statistics reported in Table~\ref{art3:tab:calibration_evidence}.
    ...
    The conformal admissibility coverage guarantee (Proposition~\ref{art3:prop:conformal}) and calibration-consistent accuracy bound (Proposition~\ref{art3:prop:calibration}) provide mathematical safety guarantees that hold regardless of data provenance: Proposition~\ref{art3:prop:conformal} requires only exchangeability (not distributional assumptions), and Proposition~\ref{art3:prop:calibration} holds for any i.i.d.\ calibration set.
...
The calibration evidence table (Table~\ref{art3:tab:calibration_evidence}) provides the traceability chain that reviewers require to assess whether the simulation parameters---and therefore the learned components' training data---are representative of real-world conditions.
...
\begin{itemize}
    \item \textbf{Small (12-cell, 2-zone)}: Primary evaluation topology, based on the Intel Lab layout (Section~\ref{art3:sec:calibration_evidence}).
...
\label{art3:sec:hyperparameters}
Table~\ref{art3:tab:hyperparameters} provides the complete hyperparameter specification for all learned components and the deterministic shield, enabling full reproducibility.
All shield parameters are set by the offline calibration procedure (Algorithm~\ref{art3:alg:calibration}); all training parameters follow standard practices for GNN-based models~\cite{hamilton2017graphsage,fey2019pyg}.

\begin{table}[t]
\centering
\caption{Complete hyperparameter specification.  Shield parameters are set by offline calibration (Algorithm~\ref{art3:alg:calibration}); training parameters follow standard GNN practices.}
\label{art3:tab:hyperparameters}
...
\label{art3:sec:training_convergence_evidence}
...
\label{art3:fig:training_curves}
\end{figure}
...
\noindent
These convergence results are empirical validation evidence for the trainability of the learned modules; they complement, rather than define, the training specification given in Section~\ref{art3:sec:training_pipeline}.
...
Each attack condition is tested at \textsc{medium} and \textsc{high} severity (Section~\ref{art3:sec:threat_model}).
...
Table~\ref{art3:tab:main_results} presents the main security results under \textsc{high}-severity attacks.
...
Table~\ref{art3:tab:cross_method} provides a consolidated cross-method comparison under the most challenging condition (combined \textsc{high}-severity attack), alongside each method's false-block rate under clean conditions.
...
\label{art3:fig:rogue_detection}
\end{figure}
...
Table~\ref{art3:tab:recovery_quality} reports $\omega_{\mathrm{rec}}$ under \textsc{high}-severity identity abuse.
...
\label{art3:tab:recovery_quality}
\begin{tabular}{lccc}
\toprule
...
The non-identity-aware baselines produce only binary block/allow decisions without any graduated response (Table~\ref{art3:tab:ablation}).

\subsubsection{RQ2 continued: Ablation analysis}

Table~\ref{art3:tab:ablation} reports the ablation results under \textsc{high}-severity identity-credential abuse.
...
\label{art3:fig:radar_comparison}
\end{figure}

\subsubsection{RQ3: Generalization across severity and seeds}

Table~\ref{art3:tab:severity} compares \textsc{medium} and \textsc{high} severity for CASTER-ZT.
...
Table~\ref{art3:tab:multiscale} reports security performance and latency across three topology scales under \textsc{high}-severity identity-credential abuse.
...
Per-tick latency increases from 3.07\,ms to 10.25\,ms at 100 cells, scaling with the number of graph edges as predicted by Proposition~\ref{art3:prop:complexity}.
...
\label{art3:fig:multiscale}
\end{figure}

\subsubsection{RQ3 continued: Threshold sensitivity analysis}

Table~\ref{art3:tab:threshold_sensitivity} reports the security--utility Pareto frontier obtained by sweeping the primary shield threshold $\gamma_1$ under \textsc{high}-severity identity abuse.
...
\label{art3:tab:threshold_sensitivity}
\begin{tabular}{ccccc}
\toprule
...
Table~\ref{art3:tab:deployment} presents the deployment profile against the 6G compliance criteria (Definition~\ref{art3:def:6g_compliance}).

\begin{table}[t]
\centering
\caption{Deployment profile: overhead and feasibility metrics (2{,}420 total runs).}
\label{art3:tab:deployment}
\begin{tabular}{lcc}
\toprule
...
Table~\ref{art3:tab:resource_budget} provides a concrete resource-budget projection for edge/gateway deployment.

\begin{table}[t]
\centering
\caption{Resource budget for edge/gateway deployment.}
\label{art3:tab:resource_budget}
\begin{tabular}{lcc}
\toprule
...
The learned components (GNN encoder, policy head, trust autoencoder, risk scorer, contrastive encoder) total $\approx$20K parameters (Table~\ref{art3:tab:training_summary}), well within the capacity of modern edge accelerators.
...
We evaluate CASTER-ZT against the three criteria of Definition~\ref{art3:def:6g_compliance} and the AI-native coverage metric of Definition~\ref{art3:def:ai_native}:
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Learned component generalization.}  The GNN encoder, trust autoencoder, and risk scorer are trained on simulated disaster episodes (Section~\ref{art3:sec:training_pipeline}).  While the training data is calibrated against real-world datasets (Table~\ref{art3:tab:calibration_evidence}), the learned components have not been validated on live network telemetry.  Transfer from simulation to real deployment may require fine-tuning or domain adaptation.  Critically, the shield's ten theoretical properties hold regardless of learned-component quality---they depend only on the deterministic shield algorithm, not on the accuracy of the upstream neural networks.
    \item \textbf{Threshold robustness.}  The threshold sensitivity analysis (Table~\ref{art3:tab:threshold_sensitivity}) shows robust performance across all $\gamma_1$ values $\ge 0.20$, with $\ge$98.2\% rogue detection and zero false positives throughout this range.  However, joint sweeps of all 14 parameters may reveal interactions not captured by single-parameter analysis.
    \item \textbf{Bounded adversary.}  The adversary cannot modify shield internals or compromise multiple zones simultaneously.  Stronger adversaries (adaptive, multi-zone) may degrade performance; the conformal guarantee (Proposition~\ref{art3:prop:conformal}) holds under exchangeability but not under adversarial distribution shift.
    ...
    \item \textbf{External baseline fidelity.}  The CPO-Soft, Shield-Binary, Agentic-Auto, and IF-Trust baselines implement the core algorithmic logic of their respective source papers~\cite{achiam2017cpo,alshiekh2018shielding,toward_autonomous_oran_2026,zahoor2025ifocsvm} within the same simulation framework, using the specific cost functions, safety specifications, confidence models, and anomaly detectors detailed in Section~\ref{art3:sec:experiments}.  Parameters were set to values recommended in the source papers (CPO's calibrated cost threshold, Alshiekh et al.'s pre-computed safety automaton, confidence-threshold ROC calibration).  However, these implementations are faithful adaptations rather than original-author code; results with original-author implementations on identical simulation parameters may differ.
...
\begin{itemize}
    ...
    \item \textbf{RQ4}: CASTER-ZT satisfies the 6G deployment criteria (Definition~\ref{art3:def:6g_compliance}) at the primary 12-cell and 36-cell scales: per-decision latency of 3.07--4.38\,ms is $\approx$2--3$\times$ below the O-RAN 10\,ms budget; at 100-cell scale, latency (10.25\,ms) marginally exceeds the budget, motivating inference optimization for larger deployments.  Model memory $\le$2\,MB fits edge-tier hardware; inference is fully local.  AI-native coverage $\eta_{\mathrm{AI}} = 5/6$ (Definition~\ref{art3:def:ai_native}) confirms that the pipeline is AI-native by design.
\end{itemize}
...
This directly validates Proposition~\ref{art3:prop:depth}: the trust assessment's action-consistency mechanism provides the primary security signal, while the authorization gate and risk scorer provide defense-in-depth for attack patterns that evade trust assessment.
...
These findings validate the positioning in Table~\ref{art3:tab:model_positioning}: the combination of graph-structured state, security-first actuation, dual-hypothesis trust--risk evaluation, formal shield with graduated outcomes, and uncertainty-based abstention is not replicated by any individual published model family.
...
The single exception is the zero-trust shield itself, which is \emph{deliberately deterministic}: its formal guarantees (Propositions~\ref{art3:prop:monotone}--\ref{art3:prop:convergence}) require that the admissibility boundary be a fixed, provable decision gate rather than a learned module whose behavior could drift or be adversarially manipulated.

\noindent\textbf{What is learned.}
The learned components, totaling $\approx$20K trainable parameters (Table~\ref{art3:tab:training_summary}), comprise:
\begin{enumerate}
...
The zero-trust shield (Algorithm~\ref{art3:alg:casterzt}) is the only non-learned component.
Its 14 scalar parameters are set by offline calibration (Algorithm~\ref{art3:alg:calibration}), not by gradient descent.
...
The title's ``AI-Native'' is formally substantiated by Definition~\ref{art3:def:ai_native}: CASTER-ZT achieves AI-native coverage $\eta_{\mathrm{AI}} = 5/6$, with every pipeline stage from perception to decision implemented as a trained neural module.
...
Third, the threshold sensitivity analysis (Table~\ref{art3:tab:threshold_sensitivity}) shows that detection performance varies with threshold setting, and the default threshold ($\gamma_1 = 0.30$) provides a reasonable precision--recall balance.
...
The conformal guarantee (Proposition~\ref{art3:prop:conformal}) and the defense-in-depth bound (Proposition~\ref{art3:prop:depth}) provide formal protection bounding the impact of the residual detection gap on overall system safety.
...
The threshold sensitivity analysis (Table~\ref{art3:tab:threshold_sensitivity}) shows remarkable robustness: the shield demonstrates robust performance across a wide range of $\gamma_1$ values, with zero false positives throughout the sweep.
...
\emph{Zero-Trust}~$\to$ NIST zero-trust boundary (Section~III-A), authorization gate (Algorithm~1 line~11), Propositions~\ref{art3:prop:authz}--\ref{art3:prop:divergence};
\emph{Shielded}~$\to$ shield algorithm (Algorithm~\ref{art3:alg:casterzt}), calibration procedure (Algorithm~\ref{art3:alg:calibration}), ten theoretical properties (Section~III-F);
\emph{Graph-Structured}~$\to$ state representation $G_t=(V_t,E_t,X_t)$ (Section~III-B), graph encoder $f_\theta(G_t)$ (Figure~\ref{art3:fig:caster_architecture}), multi-scale topologies (Table~\ref{art3:tab:multiscale});
\emph{Decision Control}~$\to$ five-outcome bounded decision space $\mathcal{D}$ (Definition~\ref{art3:def:enforcement}), graduated response (Section~IV-B);
\emph{Secure}~$\to$ threat model (Section~\ref{art3:sec:threat_model}), defense-in-depth bound (Proposition~\ref{art3:prop:depth}), 74.2\% identity-abuse detection (Table~\ref{art3:tab:main_results});
\emph{Autonomous Recovery}~$\to$ recovery policy $\pi_\theta$ (Section~III-B), recovery quality $\omega_{\mathrm{rec}}$ (Table~\ref{art3:tab:recovery_quality}), scope-reduction enforcement (Algorithm~\ref{art3:alg:scope_reduce});
\emph{AI-Native}~$\to$ formal Definition~\ref{art3:def:ai_native} ($\eta_{\mathrm{AI}} = 5/6$), five trained neural modules (Section~\ref{art3:sec:architecture}, \ref{art3:sec:training_pipeline}), AI-native coverage validated in RQ4 compliance verdict, policy-agnostic architectural contract (Section~\ref{art3:sec:ai_native});
\emph{6G}~$\to$ formal Definition~\ref{art3:def:6g_compliance} (latency, edge autonomy, compactness), O-RAN Near-RT RIC compliance at primary and medium scales (Table~\ref{art3:tab:deployment}, RQ4), 6G-driven architectural sizing (Section~\ref{art3:sec:training_pipeline}), 6G vision literature~\cite{saad2020vision6g,letaief2019roadmap6g,oran_official_architecture};
...
\begin{enumerate}
    \item \emph{Adaptive learning}: Agentic controllers update their behavior from new data, traffic patterns, and post-incident feedback.  CASTER-ZT's shield thresholds are calibrated offline and held fixed (Remark~\ref{art3:rem:convergence}); they do not learn from novel attack patterns during deployment.
...
\begin{enumerate}
    ...
    \item \emph{Formal admissibility guarantees}: Ten bounded properties including a distribution-free conformal coverage guarantee (Proposition~\ref{art3:prop:conformal}) and a formal calibration convergence rate (Proposition~\ref{art3:prop:convergence}).  Agentic systems provide no formal safety bounds on actuation.
    \item \emph{Graduated bounded decisions}: Five-outcome decision space (allow\slash scope-reduce\slash defer\slash escalate\slash block) with proportional response.  Agentic systems operate in binary execute/defer mode.
    \item \emph{Defense-in-depth composability}: Independent gating mechanisms that multiplicatively reduce unsafe execution probability (Proposition~\ref{art3:prop:depth}).  Agentic guardrails, where present, are typically single-layer confidence thresholds.
\end{enumerate}
...
Ten theoretical properties---including a defense-in-depth safety-violation bound (Proposition~\ref{art3:prop:depth}), a distribution-free conformal coverage guarantee (Proposition~\ref{art3:prop:conformal}), and a calibration convergence rate (Proposition~\ref{art3:prop:convergence})---provide formal security foundations.
...
\vspace{0.5cm}
\noindent Les hyperparam\`{e}tres complets, les preuves des dix propositions th\'{e}oriques, les r\'{e}sultats d\'{e}taill\'{e}s par \'{e}chelle et par sc\'{e}nario d'attaque, ainsi que les analyses de sensibilit\'{e} aux seuils sont fournis \`{a} l'Annexe~\ref{ann:caster-suppl}.
