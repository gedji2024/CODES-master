...
Section~\ref{art3:sec:related} reviews related work.
Section~\ref{art3:sec:framework} presents the CASTER-ZT framework, algorithms, training pipeline, and theoretical properties.
Section~\ref{art3:sec:evaluation} details the experimental evaluation.
Section~\ref{art3:sec:discussion} discusses findings and limitations.
Section~\ref{art3:sec:conclusion} concludes.

%% ======================================================================
\section{Related Work}
\label{art3:sec:related}
%% ======================================================================
...
Table~\ref{art3:tab:model_positioning} positions CASTER-ZT against the literature.
...
\begin{enumerate}
    ...
    Formal admissibility gate (Algorithm~\ref{art3:alg:casterzt}) with 14 scalar parameters set by offline calibration.  No learned weights.
\end{enumerate}
...
Algorithm~\ref{art3:alg:calibration} sets shield thresholds via offline calibration on $n$ annotated episodes.
...
All architectural hyperparameters satisfy the 6G deployment constraints of Definition~\ref{art3:def:6g_compliance}: GNN depth ($L=2$), latent dimension ($d=64$), and autoencoder bottleneck ($d_{\mathrm{in}} \to 32 \to 16 \to 8$) are sized to keep total parameters under 50K and memory under 2\,MB.
...
Under Assumption~\ref{art3:asm:standing} with $\alpha_t=1$, decreasing $\tau_t$ or increasing $\rho_t$, $u_t$, or $\Delta_t$ cannot move the output to a less conservative decision.
\end{proposition}
...
Table~\ref{art3:tab:cross_method} consolidates cross-method performance under the hardest condition (combined \textsc{high}-severity attack).
...
Fig.~\ref{art3:fig:rogue_detection} visualizes rogue detection and blocking across methods, confirming the zero-false-positive property unique to CASTER-ZT.
...
The threshold sensitivity analysis (Table~\ref{art3:tab:threshold_sensitivity}) confirms robustness: zero false positives across all $\gamma_1$ values $\in\{0.10,\ldots,0.60\}$.
...
All four criteria of Definition~\ref{art3:def:6g_compliance} are satisfied at 12-cell and 36-cell scales.
At 100-cell, latency (10.25\,ms) marginally exceeds the budget, motivating inference optimization.
Fig.~\ref{art3:fig:training_curves} confirms rapid convergence for all four learned components.
Fig.~\ref{art3:fig:radar_comparison} provides a multi-metric radar comparison across methods.
Fig.~\ref{art3:fig:multiscale} depicts the scaling behaviour of detection accuracy and latency.

%% ======================================================================
\section{Discussion and Limitations}
\label{art3:sec:discussion}
%% ======================================================================
...
This validates Proposition~\ref{art3:prop:depth}: independent gates multiplicatively reduce unsafe execution probability.
...
\vspace{0.5cm}
\noindent Les hyperparamètres complets, les preuves des dix propositions théoriques, les résultats détaillés par échelle et par scénario d'attaque, ainsi que les analyses de sensibilité aux seuils sont fournis à l'Annexe~\ref{ann:caster-suppl}.
