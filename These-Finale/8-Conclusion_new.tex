...
Le point de départ de cette thèse était simple à énoncer, difficile à résoudre~: comment faire fonctionner un réseau de capteurs sans fil dans les décombres d'une catastrophe, avec des nœuds détruits, des batteries qui s'épuisent, une empreinte carbone à maîtriser, et des politiques d'IA qui peuvent être compromises~? Aucun protocole existant ne répondait à plus de quatre des douze critères que nous avions identifiés. Ce chapitre fait le point sur ce que nous avons accompli (Section~\ref{sec:concl-synthese}), sur les retombées concrètes pour les entreprises, la société et les gouvernements (Section~\ref{sec:concl-retombees}), sur les limites honnêtes du travail (Section~\ref{sec:concl-limitations}), et sur les pistes de recherche que ces travaux ouvrent (Section~\ref{sec:concl-futures}).
...
\textbf{Article 1 --- GAPF (Article~1,~\cite{gapf_tgcn2026}).}
...
L'Article~1 (GAPF) a été publié dans IEEE Transactions on Green Communications and Networking~\cite{gapf_tgcn2026}, revue classée Q1 selon le SCImago Journal Rankings, ce qui valide la contribution dans un forum de premier plan pour les thématiques d'efficacité énergétique dans les réseaux de communication.
...
Six contributions originales se dégagent de l'ensemble de ces travaux.

\textbf{Contribution 1~: Premier cadre de fusion de politiques MARL via GNN pour le routage dans les WSN.}
GAPF (Article~1,~\cite{gapf_tgcn2026}) est, à notre connaissance, le premier méta-contrôleur qui exploite un encodeur GCN pour capturer la structure topologique d'un WSN et sélectionner dynamiquement la politique MARL la plus adaptée. Avec seulement 1\,473 paramètres entraînables ($26\times$ moins qu'un seul agent RNN), GAPF démontre que la fusion de politiques peut être plus efficace que l'entraînement d'une politique monolithique unique, ouvrant une nouvelle direction en méta-apprentissage pour les systèmes multi-agents. La portée de cette contribution dépasse les WSN~: le paradigme GNN+CAS est applicable à tout système multi-agent où des experts complémentaires coexistent.

Des travaux publiés en 2024--2026 confirment la pertinence de cette direction~: DDWQMIX~\cite{ddwqmix_cac2024} propose une extension de QMIX améliorant la robustesse dans les environnements non stationnaires, mais sans encodage GNN de la topologie ni méta-contrôleur de sélection dynamique. Ces travaux confirment l'intérêt des extensions QMIX sans pour autant converger vers la solution légère et topologiquement consciente de GAPF.
...
La métrique LCI intègre le carbone incorporé, opérationnel et de fin de vie dans un indicateur unique exprimé en gCO$_2$eq par paquet utile livré. Cette métrique comble une lacune fondamentale dans l'évaluation des protocoles de routage~: les métriques existantes (Joules/paquet, durée de vie) ne reflètent pas l'impact environnemental réel. Le LCI offre un cadre de comparaison équitable qui pourrait être adopté par la communauté pour l'évaluation de tout protocole IoT.
...
L'intégration de CADR, CARE, SHDR et LSE dans un cadre unifié avec une preuve mathématique de la borne de Pareto CDL est, à notre connaissance, inédite dans la littérature. La borne prouve que CALASH converge simultanément vers un taux de livraison quasi-optimal \emph{et} un budget carbone borné, fournissant les premières garanties théoriques sur la durabilité carbone d'un protocole de routage.

Les travaux récents de Vaccaro et al.~\cite{vaccaro_sustainable2025} sur les centres de données durables et la méthodologie AIOTI~\cite{aioti2024carbon} pour la mesure de l'empreinte carbone IoT confirment la tendance vers une comptabilité carbone holistique~; aucun de ces travaux n'intègre cependant cette comptabilité dans les décisions de routage d'un protocole WSN.
...
L'attaque SUB-PLAY~\cite{subplay_marl2025} (2024), qui démontre la vulnérabilité des systèmes MARL coopératifs à des agents adversariaux contrôlant un sous-ensemble d'agents, renforce a posteriori la nécessité de la couche de vérification CASTER-ZT. De même, les avancées récentes en ZTA pour l'IoT~\cite{shakya_zt_iot2025, kaur_zta_iot2024} progressent vers des architectures de confiance zéro adaptées aux dispositifs contraints, mais sans décisions graduées ni garanties formelles dans le budget O-RAN.
...
Ces contributions, aussi prometteuses soient-elles, reposent sur des hypothèses et des choix de modélisation qui méritent d'être discutés sans détour.
...
\begin{enumerate}[resume]
    ...
    \item \textbf{Extension aux paradigmes émergents de MARL.} Les avancées récentes en MARL --- notamment DDWQMIX~\cite{ddwqmix_cac2024}, PPS-QMIX~\cite{pps_qmix_arxiv2024} et CoMIX~\cite{comix_arxiv2024} --- ouvrent des opportunités pour enrichir le portefeuille d'experts de GAPF au-delà du duo QMIX/QTRAN. L'intégration de DDWQMIX comme troisième expert pour les environnements non stationnaires serait une extension naturelle.
    \item \textbf{Standardisation de la métrique LCI.} La soumission de la métrique LCI aux organismes de normalisation ISO TC 207/SC 7 (ISO~14067 pour les empreintes carbone des produits) permettrait son adoption comme standard d'évaluation pour les protocoles IoT, consolidant ainsi la Contribution~3 de cette thèse.
...
Ces travaux ont des implications concrètes pour plusieurs organismes~: ISO TC 207/SC 7 (extension de ISO~14067 aux protocoles IoT via le LCI), ETSI ISG ENI (réseaux auto-gérés par AI), O-RAN Alliance WG2 (GAPF/CALASH comme rApps, CASTER-ZT comme xApp), et 3GPP SA5 (mécanismes de confiance AI/ML pour le 6G). Pour les gouvernements, la pile contribue directement au Cadre de Sendai \cite{UNDRR2015Sendai} (indicateurs C-2 et C-6) et peut être intégrée sans modification matérielle sur les RIC O-RAN existants.
...
Le Tableau~\ref{tab:bilan-global} récapitule les résultats quantitatifs clés, mettant en perspective l'ampleur des gains par rapport à l'état de l'art et le coût computationnel associé.
...
Les catastrophes naturelles continueront de frapper avec une intensité et une fréquence croissantes dans un monde affecté par le changement climatique \cite{IPCC2021AR6}. La surveillance en temps réel par les réseaux de capteurs sans fil constitue un maillon essentiel de la chaîne de prévention et de réponse \cite{UNDRR2015Sendai}. Ce que nous espérons avoir démontré ici, c'est que l'intelligence artificielle --- lorsqu'elle est conçue avec parcimonie, conscience environnementale et vérification systématique --- peut constituer un outil fiable pour les environnements les plus critiques. GAPF, CALASH et CASTER-ZT devront être validés sur matériel réel pour confirmer leur viabilité opérationnelle. C'est l'étape décisive que nous appelons de nos vœux pour les prochaines années.
