...
L'Article~1 (GAPF) a été publié dans IEEE Transactions on Green Communications and Networking~\cite{gapf_tgcn2026}, revue classée Q1 selon le SCImago Journal Rankings, avec un facteur d'impact reflétant l'importance croissante des communications vertes et durables. Cette publication valide la contribution dans un forum de premier plan pour les thématiques d'efficacité énergétique dans les réseaux de communication.
...
Des travaux publiés en 2024--2026 confirment la pertinence de cette direction~: DDWQMIX~\cite{ddwqmix_cac2024} et POWQMIX~\cite{powqmix_arxiv2025} proposent des extensions de QMIX améliorant la robustesse, mais sans encodage GNN de la topologie ni méta-contrôleur de sélection dynamique~; SoCo~\cite{soco_arxiv2025} développe la fusion de politiques pour des agents Solo-to-Collaborative, mais dans un contexte sans contraintes d'embarqué WSN. Ces travaux confirment l'intérêt de la fusion de politiques MARL mais ne convergent pas vers la solution légère et topologiquement consciente de GAPF.
...
\begin{enumerate}
    ...
    Cette limitation est d'autant plus importante que les interactions entre GAPF et CASTER-ZT --- en particulier l'impact du bouclier sur la sélection d'expert CAS lorsque certaines actuations sont bloquées ou différées --- pourraient modifier le comportement de convergence de GAPF décrit dans l'Article~1~\cite{gapf_tgcn2026}. Des protocoles d'intégration progressive (déploiement de GAPF seul, puis ajout de CALASH, puis de CASTER-ZT) permettraient de caractériser ces interactions de manière contrôlée.
...
\begin{enumerate}[resume]
    ...
    \item \textbf{Extension aux paradigmes émergents de MARL.} Les avancées récentes en MARL --- extensions de QMIX~\cite{ddwqmix_cac2024, powqmix_arxiv2025}, QTRAN++~\cite{qtranpp_arxiv2020}, cadres de fusion de politiques (SoCo~\cite{soco_arxiv2025}) --- ouvrent des opportunités pour enrichir le portefeuille d'experts de GAPF au-delà du duo QMIX/QTRAN. L'intégration de DDWQMIX comme troisième expert pour les environnements non stationnaires serait une extension naturelle.
