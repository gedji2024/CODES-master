...
Networking} (IEEE TGCN).\footnote{%
    IEEE TGCN est une revue à comité de lecture arbitrée par des pairs, indexée dans
    les bases de données suivantes~: \textbf{Scopus} (SJR~$\approx$~1.8,
    Q1 en ingénierie et informatique)~; \textbf{Web of Science} (Science Citation Index
    Expanded, JIF~$\approx$~7{,}7, Journal Citation Reports 2024)~; \textbf{CORE}
    (classée~A, ERA~2023). Sa thématique couvre les communications vertes et les
    réseaux de nouvelle génération, en fort alignement avec les contributions de
    cette thèse (routage écoénergétique, durabilité carbone, intégration 6G). La
    soumission de l'Article~1 à cette revue résulte d'une décision éditoriale mûrement
    réfléchie, prenant en compte l'impact facteur, l'adéquation thématique et la
    visibilité internationale de la publication. Le DOI définitif sera ajouté dès
    son attribution par l'éditeur (en cours, Mai~2026).%
}

\smallskip
\noindent\textbf{Contribution de l'auteur~:} L'auteur est l'unique concepteur de
l'architecture GAPF (encodeur GCN, contrôleur CAS, réseau d'attention Q$\cdot$K),
...
Control for Disaster-Monitoring Sensing Networks (CASTER-ZT)}
