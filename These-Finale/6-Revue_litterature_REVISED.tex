...
\begin{itemize}
    \item \textbf{Temps de convergence élevé}~: typiquement \hl{$O(n^2)$ à $O(n^3)$ par itération}~\cite{dorigo2004ant,kennedy1995pso}\hl{,} incompatible avec les contraintes de temps réel~;
...
\begin{itemize}
    ...
    \item \textbf{Calcul}~: l'inférence doit s'exécuter en temps réel \hl{(typiquement $<$ 100~ms}~\cite{Warden2020TinyML}\hl{)} sans unité de calcul en virgule flottante (FPU) sur les Cortex-M0/M3, nécessitant une arithmétique en point fixe.
...
\begin{itemize}
    ...
    \item \textbf{Carbone de fin de vie}~: traitement des déchets électroniques, recyclage partiel. \hl{Estimé à $\sim$500\,gCO$_2$eq par n\oe{}ud, ajusté au taux de recyclage effectif}~\cite{pirson2021embodied}\hl{.}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Gain de beamforming passif}~: \hl{le gain effectif d'un RIS à $M$ éléments est approximé par $G_{\text{RIS}} \approx M^2 \cdot G_{\text{elem}}$ en conditions de phase optimale}~\cite{huang2021ris}\hl{,} offrant un gain quadratique sans consommation RF supplémentaire~;
    \item \textbf{Efficacité énergétique}~: les RIS ne requièrent pas d'amplificateurs RF actifs, \hl{leur consommation étant limitée au circuit de contrôle des déphaseurs ($\sim$5\,mW pour 64 éléments)}~\cite{huang2021ris}\hl{.}
\end{itemize}
