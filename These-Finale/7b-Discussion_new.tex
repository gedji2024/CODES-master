...
\medskip
La question Q1 demande si un méta-contrôleur de fusion de politiques MARL via un encodeur GNN peut surpasser les politiques individuelles tout en respectant les contraintes de déploiement embarqué.

Les résultats de l'Article~1 (GAPF) répondent affirmativement à cette question~: le méta-contrôleur GCN+CAS atteint un PDR de 98--99\,\% sur les 7~scénarios évalués, soit une amélioration de 43--111\,\% par rapport à QMIX seul (46--67\,\%) et à QTRAN seul (46--68\,\%). La consommation d'énergie est réduite de 2 à 3{,}5$\times$ par rapport aux références. Avec seulement 1\,473~paramètres ($\approx$5{,}8~KB en FP32, $\approx$1{,}5~KB en INT8), GAPF est compatible avec les microcontrôleurs ARM Cortex-M0 --- une contrainte de déploiement que les agents QMIX ($\approx$39K paramètres, 152~KB) ne satisfont pas. La sélection dynamique d'expert par CAS démontre que la complémentarité entre QMIX (stable sous faible densité) et QTRAN (plus expressif sous forte densité) peut être exploitée de manière adaptative~\cite{rashid2018qmix,son2019qtran}. Ces résultats confirment l'hypothèse centrale~: la fusion de politiques via GNN est plus efficace qu'une politique monolithique, et ce à un coût computationnel $26\times$ inférieur. La réponse à Q1 est donc \textbf{affirmative}.
...
\medskip
La question Q2 demande si un protocole de routage WSN peut intégrer les trois phases du cycle de vie carbone (ISO~14040) dans ses décisions tout en maintenant un niveau de service acceptable.

CALASH (Article~2) apporte une réponse affirmative et quantifiée~: la métrique LCI, conforme à ISO~14040~\cite{iso14040}, intègre le carbone incorporé ($\approx$10\,000~gCO$_2$eq/nœud~\cite{pirson2021embodied}), opérationnel et de fin de vie dans un indicateur unique en gCO$_2$eq/paquet. Sur 30~graines Monte Carlo, CALASH réduit le LCI de 33\,\% (de 13{,}42 à 8{,}99~gCO$_2$eq/paquet) par rapport à LEACH --- la première démonstration publiée d'une réduction mesurable du LCI carbone pour un protocole de routage WSN. La durée de vie réseau augmente de 60\,\% (de 931 à 1\,486~tours) grâce au pilier SHDR, et le PDR se maintient à 82{,}9\,\%. Les garanties de Lyapunov~\cite{neely2010lyapunov} prouvent formellement la convergence vers un voisinage $O(1/V)$ de l'optimum avec budget carbone borné. L'aperçu clé --- que la réduction du volume de données transmises via la détection comprimée (CADR) constitue le levier principal de réduction du carbone opérationnel, complémentaire à l'extension de durée de vie qui amortit le carbone incorporé --- n'avait pas été démontré avant CALASH. La réponse à Q2 est donc \textbf{affirmative}.
...
\medskip
La question Q3 demande si un cadre de contrôle de décision conforme au paradigme zéro-confiance peut, simultanément, détecter les politiques voyous, émettre des décisions graduées et fournir des garanties formelles dans le budget de latence O-RAN.

CASTER-ZT (Article~3) apporte une réponse affirmative sur tous les axes~: la campagne de 2\,420~exécutions démontre une détection de politiques voyous de 74{,}2--98{,}3\,\% (selon le type d'attaque), avec un taux de faux positifs exactement nul --- garanti par construction via les 10~propositions formelles du bouclier~\cite{rose2020nist}. Le score de récupération composite $\omega_{\text{rec}} = 0{,}733$ est 3{,}7 à 5{,}2$\times$ supérieur aux références CPO et Shield-Binary. La latence de 3{,}07~ms par décision est 3{,}3$\times$ inférieure au budget de 10~ms du near-RT RIC O-RAN~\cite{polese2023oran}. Le spectre de cinq décisions graduées (autoriser, réduire, différer, escalader, bloquer) permet une dégradation gracieuse qui maintient la mission de surveillance même sous attaque, une propriété que les boucliers binaires existants~\cite{alshiekh2018shielding} ne peuvent pas offrir. La menace adversariale multi-agent récemment démontrée par SUB-PLAY~\cite{subplay_marl2025} confirme la pertinence de cette couche de vérification. La réponse à Q3 est donc \textbf{affirmative}.
...
À notre connaissance, peu de travaux dans la littérature proposent des extensions de QMIX intégrant à la fois un encodeur GNN et une métrique de cycle de vie carbone. DDWQMIX~\cite{ddwqmix_cac2024} et POWQMIX~\cite{powqmix_arxiv2025} constituent des avancées notables dans la famille QMIX --- le premier en introduisant une pondération dynamique des différences pour améliorer la stabilité d'entraînement, le second en explorant des politiques optimistes pondérées --- mais ni l'un ni l'autre n'intègre d'encodage GNN de la topologie réseau ni de métrique LCI dans la fonction de récompense. De même, SoCo~\cite{soco_arxiv2025} propose un cadre de fusion de politiques coopératives récent, mais sans application aux WSN ni encodage topologique par graphe~: son architecture n'exploite pas la structure spatiale du réseau de capteurs, ce qui limite son applicabilité directe à notre contexte.

Sur le volet sécurité, Shakya et al.~\cite{shakya_zt_iot2025} et Kaur et al.~\cite{kaur_zta_iot2024} représentent des avancées significatives de l'architecture zéro-confiance (ZTA) pour l'IoT. Cependant, ces travaux se concentrent sur l'authentification et le contrôle d'accès au niveau du réseau, sans proposer de décisions graduées à cinq niveaux ni de garanties formelles de latence compatibles avec le budget de 10\,ms du near-RT RIC O-RAN~\cite{polese2023oran}. CASTER-ZT se distingue donc par son application à la couche de décision des agents autonomes et par ses preuves formelles de correction. La menace adversariale multi-agent récemment démontrée par l'attaque SUB-PLAY \cite{subplay_marl2025} confirme en outre la nécessité d'une couche de vérification telle que CASTER-ZT.


%%
%%  CALASH ET ECONOMIE CIRCULAIRE
%%
\subsection{CALASH et économie circulaire}\label{sec:disc-circular}

Une question légitime porte sur la relation entre CALASH et les principes de l'économie circulaire~: la métrique LCI, en intégrant les coûts de fin de vie (DEEE), s'inscrit-elle dans ce cadre ?

La réponse est affirmative, avec des nuances importantes. CALASH adopte une perspective de comptabilité carbone du cycle de vie~\cite{iso14040} qui est structurellement alignée avec les principes de l'économie circulaire~: (i)~en maximisant la durée de vie utile des capteurs (+60\,\% de tours de fonctionnement), CALASH réduit le taux de renouvellement et donc les émissions liées à la fabrication de nouveaux nœuds~; (ii)~en intégrant le carbone de fin de vie dans la décision de routage, CALASH incite à utiliser préférentiellement les nœuds dont le bilan fin de vie est le plus favorable (taux de recyclage élevé, moindre impact DEEE). Cependant, CALASH n'aborde pas la conception des nœuds capteurs eux-mêmes pour la recyclabilité, ni les systèmes de collecte et de recyclage en fin de vie --- des dimensions relevant de la chaîne d'approvisionnement et de la conception éco-responsable des produits, au-delà du périmètre d'un protocole de routage. Dans ce sens, CALASH est un artefact de \textbf{durabilité opérationnelle et computationnelle}, complémentaire mais non substitut à une démarche d'économie circulaire appliquée au matériel des capteurs.
